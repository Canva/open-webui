...
> **Scope:** dev-loop only. Zero changes to the rebuild's runtime backend, runtime image, or production deploy. The deliverable is a new compose service (`agent-platform`) plus a small Ollama sibling that gives `make dev` a working agent dropdown and streaming chat without anyone needing to set an OpenAI token.

> **Terminology.** The rebuild's product domain calls each surfaced entry an "agent" — internally each agent has a preselected underlying model. The agent platform's external HTTP surface keeps the OpenAI wire format (`/v1/models` path; `model` field on the request body / chunk envelope) unchanged for SDK compatibility, but its internal types and the rebuild's API both use "agent" — see § Component-by-component for the seam. The original review of this plan still occasionally talks about a "model dropdown" because the OpenAI wire type is named `Model`; in the running product the user-facing widget is the agent selector.

## Goal

After this lands, a developer who has just cloned the repo runs `make dev`, opens `http://localhost:5173`, and immediately sees a populated agent dropdown with one local agent (`dev`, backed internally by `qwen2.5:0.5b`). They pick it, type "hello", and watch tokens stream into the conversation view exactly as they would with a real internal agent gateway. Free-form prompts work. Streaming, cancel, persistence, the SSE event taxonomy, and the agent-validation 400 path are all exercised against a real OpenAI-compatible upstream — only the upstream is local Ollama instead of the production gateway.

The runtime backend is **not modified**. `OpenAICompatibleProvider` does not learn about Ollama, Pydantic AI, or any new agent id. It continues to talk OpenAI HTTP at `AGENT_GATEWAY_BASE_URL`; the only thing that changes is what's listening on the other end of that URL in the dev compose stack. This preserves the locked single-provider constraint from [rebuild.md § 2](../../../rebuild.md) and [FastAPI-best-practises.md § B.4](../best-practises/FastAPI-best-practises.md).
...
The naming (`agent-platform`, not `dev-gateway`) is forward-looking: today the service ships one Pydantic-AI `Agent` per surfaced agent id with no system prompt and no tools, but the registry shape (`AGENTS: list[AgentDef]` with one Agent per entry) supports multiple personas / tool-using agents on top of the same underlying model tag without touching the OpenAI-compat translation layer. M2 ships one agent; future surfaces (M5 dev-mode automations, smoke harnesses) extend the catalogue by adding `AgentDef` entries.
...
- Compose additions in [rebuild/infra/docker-compose.yml](../../infra/docker-compose.yml): two new services (`ollama`, `agent-platform`); `app` service gains `AGENT_GATEWAY_BASE_URL=http://agent-platform:8081/v1` and `depends_on: agent-platform`. The `ollama` service uses an entrypoint wrapper that fronts `ollama serve` with a one-shot `ollama pull qwen2.5:0.5b` so backing-tag availability and daemon readiness are gated by a single healthcheck — see § Compose wiring for the rationale of folding the pull into the daemon entrypoint instead of running it as a separate init container.
- One Makefile target tweak: `make dev` continues to bring the whole stack up (no new target needed); the existing `make verify-stack` target is extended to curl `/api/agents` and assert at least one agent is returned. Cold-start `verify-stack` now pays the one-time `qwen2.5:0.5b` pull (~400 MB) so the wall-clock budget for a fresh CI runner grows from ~30 s to ~3-5 minutes; subsequent runs (cached layer + cached `ollama_models` volume) stay at the existing budget. Document this in the Makefile target's leading comment so a future "verify-stack got slower" investigation lands at the right cause.
- One paragraph **appended below** the existing `AGENT_GATEWAY_BASE_URL` / `AGENT_GATEWAY_API_KEY` block in [rebuild/backend/.env.example](../../backend/.env.example) documenting that the dev compose stack provides this automatically and how to override it (e.g. point at a real internal gateway, LM Studio, or vLLM). The existing two-line block stays — it documents the prod sidecar contract and the `SecretStr` semantics on `AGENT_GATEWAY_API_KEY`; the new paragraph adds the dev-loop story without touching the prod documentation.
- Optional standalone `make dev-rebuild-models` target for refreshing the local Ollama backing-tag cache on demand (recreates the `ollama` container so its entrypoint wrapper re-runs the pull on next boot — `ollama pull` itself is idempotent on cache hit, so this is mostly useful when the named volume has been wiped or a backing-tag spec changed). The target name keeps `models` to match the `ollama list`/`ollama pull` vocabulary; "model" here is Ollama's term for a stored tag, not the rebuild's agent.
- Platform-side unit tests at [rebuild/infra/agent-platform/tests/](../../infra/agent-platform/tests/) covering: the OpenAI request → Pydantic AI translation, the SSE chunk emitter, the agent registry, and the `/v1/models` shape (the path keeps the OpenAI wire name). **Run independently of the backend pytest suite** via a separate `uv run --project infra/agent-platform pytest` invocation; not wired into `make test-unit`.
- One smoke test at [rebuild/infra/agent-platform/tests/test_smoke_compose.py](../../infra/agent-platform/tests/test_smoke_compose.py), pytest-marked `@pytest.mark.compose`, that brings up the full stack, curls `GET http://localhost:8080/api/agents` (through the rebuild's app, not directly), and asserts the configured agent ids appear. **Skipped by default**; runs under `make verify-stack`.
- Brief plan-keeper update to [rebuild.md](../../../rebuild.md) § 2 (Target architecture) noting that the dev compose stack ships a local OpenAI-compatible upstream so agent discovery works out-of-the-box. The locked sentence ("OpenAI-compatible only, single provider. The internal agent gateway is the sole upstream") is unchanged — the agent platform is upstream infra (same category as MySQL and Redis), not a second provider class in the runtime backend.
- One-line `label` plumbing in [rebuild/backend/app/providers/openai.py](../../backend/app/providers/openai.py): change line 110 from `label=m.id` to `label=getattr(m, "label", None) or m.id` so non-id labels emitted by an OpenAI-compatible upstream survive the round trip into the rebuild's `AgentInfo.label` field. The OpenAI Python SDK's `Model` object (a wire-format type — the SDK still names it `Model` even though the rebuild and the agent platform both call each entry an "agent") already accepts arbitrary extra fields and exposes them via attribute access, so the `getattr` shim is forward-compatible with both the agent platform (which emits `label`) and the production internal gateway (which doesn't, today, and falls through cleanly to `m.id`). This is the **only** change the feature plan makes under `rebuild/backend/`; tightly scoped, zero-risk, and required for the friendly label to actually surface in the dropdown — without it the dev user sees `dev` instead of `Dev (Qwen 2.5, 0.5B)`. Covered by a one-line addition to the existing [rebuild/backend/tests/integration/test_agents_cache.py](../../backend/tests/integration/test_agents_cache.py): a `cassette_mock_app` whose `/v1/models` returns an entry with both `id` and `label` should round-trip the `label` through `cache.get()`.
...
| Option | Why not |
|---|---|
| **A. Extend the cassette mock** to serve a stub echo on cassette-miss | Couples test fixtures to dev runtime. The mock has no concept of a real LLM and cannot answer free-form prompts plausibly. Conflates two purposes. |
| **B. Point `AGENT_GATEWAY_BASE_URL` straight at vanilla Ollama's `/v1`** | Ollama already speaks OpenAI HTTP, so this works mechanically. But: agent ids are then whatever tag was pulled (e.g. `qwen2.5:0.5b`), the dropdown is polluted with every random model the developer ever pulled into Ollama, and there is no place to land future affordances (per-agent system prompts, Pydantic-AI tool wiring, friendly labels, multi-persona behaviour). |
| **C. Pydantic-AI agent platform in front of Ollama (this plan)** | One narrow service that owns the curated agent catalogue, the friendly labels, the OpenAI wire format, and a Pydantic AI `Agent` per surfaced entry. Future features (M5 dev-mode automations, smoke harnesses) ride on the same surface. Costs one small image and one config file. |

The reference for the Pydantic AI → OpenAI compat translation is [`samreay-agents-platform/packages/ui/src/ui/oai_router.py`](file:///Users/samreay/repos/samreay-agents-platform/packages/ui/src/ui/oai_router.py); we pull the streaming `agent.iter()` shape, the `ChatCompletionChunk` emitter, and the `/v1/models` response shape from there (the path stays on the OpenAI wire name), but we **do not** import that module — it has agent-registry coupling we don't need. Translate the patterns, ship a fresh implementation here.
...
- The rebuild's `app` service gets two changes: `AGENT_GATEWAY_BASE_URL: http://agent-platform:8081/v1` in `environment:`, and `agent-platform: { condition: service_healthy }` added to `depends_on:`.
- **Why the pull lives in the `ollama` entrypoint, not a separate `ollama-pull` init container.** A separate init container would surface a slightly cleaner failure mode for pull errors (a distinct exit code with its own log stream), but it triples the service count and adds a `condition: service_completed_successfully` dependency that compose v2 still warns about. Folding the pull into the daemon entrypoint with a backing-tag-grep healthcheck collapses both signals (daemon up + tag present) into one and makes the topology two services instead of three. Pull failures (network / disk-full) surface as a healthcheck timeout, which prints the daemon's stderr in `docker logs rebuild_ollama` — discoverable, just not as labelled.
- **The agent id surfaced through `GET /api/agents` is a stable alias** (M2 ships `dev`; future personas land as `dev-pirate`, `dev-coder`, …), not the underlying Ollama tag. The mapping lives in the platform's config and is the only place that knows about `qwen2.5:0.5b`. This shields the rebuild's frontend from "what tag did the developer happen to pull" churn and makes the future "multiple agents, one underlying model" trajectory a config edit, not a code edit.
- **Friendly labels** (`label: "Dev (Qwen 2.5, 0.5B)"`) come from the same platform config and surface through the rebuild's existing `AgentInfo.label` field ([rebuild/backend/app/schemas/agent.py](../../backend/app/schemas/agent.py)). The provider's `list_agents()` currently sets `label = m.id`; this PR ships the one-line `getattr(m, "label", None) or m.id` plumbing alongside the agent-platform service so the friendly label is visible in the dropdown on day one. See the matching Deliverables bullet for the exact diff.
...
rebuild/
  infra/
    ...
    agent-platform/
      README.md                        # one-pager: what this is, how to swap agents, why it's not in tests
      ...
      app/
        ...
        config.py                      # Settings: OLLAMA_BASE_URL, AGENTS (json), HOST, PORT, LOG_LEVEL
        oai_router.py                  # /v1/models, /v1/chat/completions (stream + non-stream) — wire path keeps OpenAI name
        oai_models.py                  # OpenAI wire schemas: ChatCompletionRequest/Response/Chunk + ModelInfo/List
        agents.py                      # Pydantic-AI Agent factory keyed by agent alias
      tests/
        __init__.py
        ...
        test_oai_router_models.py      # /v1/models returns the configured agent aliases on the OpenAI wire shape
        ...
        test_agents.py                 # agent alias -> ollama tag mapping, unknown alias raises 404
        test_smoke_compose.py          # @pytest.mark.compose: bring up the stack, curl through rebuild app
  backend/
    .env.example                       # MODIFY: add paragraph under AGENT_GATEWAY_BASE_URL
  Makefile                             # MODIFY: extend verify-stack to curl /api/agents and assert non-empty
```
...
Standard `Settings(BaseSettings)`, mirroring the rebuild's settings convention per [m0-foundations.md § Settings(BaseSettings) "Casing convention (locked)"](m0-foundations.md#settingsbasesettings) — PEP 8 lowercase attribute names with `case_sensitive=False`, env-var keys (`OLLAMA_BASE_URL`, `AGENTS`, etc.) stay UPPER_SNAKE.
...
class Settings(BaseSettings):
    ...
    # var holding a JSON list of AgentDef shapes (case_sensitive=False
    # above bridges the UPPER_SNAKE env-var name to this lowercase
    # attribute).
...
`AGENTS` (env var) / `agents` (Python attribute) is overridable from compose so an individual developer can add e.g. a coder persona without editing the platform's source. Compose default ships exactly one entry; future personas land as additional `AgentDef` entries pointing at the same `qwen2.5:0.5b` tag with different ids and labels (and, when the API surfaces it, different system prompts).

### `infra/agent-platform/app/agents.py`

One Pydantic-AI `Agent` per configured agent alias, built once at startup and cached on `app.state.agents`. `pydantic-ai==1.88.0`'s `OpenAIModel` constructor accepts an `OpenAIProvider(base_url=...)` so we point each Agent at the local Ollama daemon's `/v1` endpoint. (`OpenAIModel` is the pydantic-ai driver type — it represents the underlying LLM model behind one of our agents.)
...
def build_agents(settings: Settings) -> dict[str, AgentEntry]:
    """Construct one Pydantic-AI ``Agent`` per configured agent.
...
Locked: **no system prompt is set on the Agent.** The rebuild's streaming pipeline already prepends any `params.system` into `messages[0]` before the HTTP call (see `rebuild/backend/app/providers/openai.py::OpenAICompatibleProvider.stream` lines 121-123); a system prompt baked into the Agent here would double-stack. When future `AgentDef` entries want a persona-style system prompt, the `Agent(...)` constructor takes a `system_prompt=` kwarg — the implementer wires it through then, gated on a new optional `AgentDef.system_prompt` field, with a comment pointing at this paragraph as the rationale for why M2 ships without it.

### `infra/agent-platform/app/oai_models.py`

Pydantic schemas for the OpenAI wire shape. (The file name keeps the `oai_models.py` pattern because everything in it mirrors OpenAI's wire types — including the `ModelInfo` / `ModelListResponse` names retained for SDK compatibility. The rebuild's product domain calls each surfaced entry an "agent"; this file is the OpenAI translation layer where the wire field names stay on `model`.) Crib directly from the reference `oai_models.py` with the slimming pass below — the reference carries `ConfigDict(extra="allow")` because Open WebUI sends extra fields; we accept the same since the OpenAI SDK adds telemetry headers and the occasional `stream_options` field.
...
`ModelInfo.label` is **not** standard OpenAI — the OpenAI public spec for `Model` only defines `id`, `object`, `created`, `owned_by`. (The wire-format type stays named `ModelInfo` even though the rebuild and platform both call each entry an "agent" internally — keeping the name aligns with OpenAI SDK deserialisation.) The OpenAI Python SDK (which the rebuild's provider uses as transport) still exposes any extra wire fields via attribute access, so the rebuild's `OpenAICompatibleProvider.list_agents` reading `getattr(m, "label", None)` (the one-line plumbing change shipped by this PR — see § Deliverables) lights the friendly label up cleanly. The production internal gateway doesn't emit `label` today; the `getattr` shim falls through to `m.id` for that case so existing prod behaviour is unchanged.
...
async def _stream_response(
    ...
    try:
        ...
        # ``model_id`` is the wire-format name on the OpenAI request
        # body; in-process this is the rebuild's agent id.
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # time instead of at first /api/agents call. Five attempts at 2 s
...
The diff to [rebuild/infra/docker-compose.yml](../../infra/docker-compose.yml) — two new top-level services, a new named volume, and two changes to the `app` block. The `ollama` service's healthcheck gates on the configured backing tag being *present in `ollama list`*, not just on the daemon being up — so when `ollama` is reported healthy, the tag is already cached and the agent platform can stream against it immediately.

> **Locked: use the in-image `ollama` CLI for both the readiness loop and the healthcheck — never `curl`.** The official `ollama/ollama:0.5.7` image ships **no HTTP client at all** (no `curl`, no `wget`, no `nc`, no `python`). A `curl`-based healthcheck fails immediately with `sh: 1: curl: not found` and the container is marked `unhealthy`; worse, a `curl`-based readiness loop in the entrypoint spins forever, so `ollama pull` never runs and the tag is never cached. `ollama list` exits 0 once the daemon's local socket is up and piping it to `grep -q "<tag>"` produces a stable single-signal oracle (daemon-up AND tag-cached). Both tools (`ollama`, `grep`) are in-image, so no custom Dockerfile is needed.
...
services:
  ...
  ollama:
    ...
    command:
      - |
        ...
        wait "$$pid"
    healthcheck:
      # Greps the backing tag out of `ollama list` so "healthy" implies both
      # daemon-up AND tag-cached. The agent platform's depends_on chain
  ...
  app:
    ...
    environment:
      ...
      AGENT_GATEWAY_BASE_URL: http://agent-platform:8081/v1   # <-- NEW
...
The agent-platform service is **always brought up**, even if a developer overrides `AGENT_GATEWAY_BASE_URL` to point at a real internal gateway. The agent-platform image is small and the `qwen2.5:0.5b` pull is a one-time ~400 MB cost; standing it up unconditionally keeps the compose topology static and avoids `--profile` flag wrangling. Document the first-time pull cost in the agent-platform `README.md` so the slow first-up is expected, not surprising.

## Settings additions

The rebuild's `Settings` class is **not** modified — the only change is the value of `AGENT_GATEWAY_BASE_URL` injected by compose. This is exactly the pattern locked in [m0-foundations.md § Settings](m0-foundations.md#settingsbasesettings) for environment-driven config.
...
# curated set of agents (`dev`, backed internally by `qwen2.5:0.5b`)
# so the UI works out of the box with no external token. Override
# here to point at the real internal agent gateway, LM Studio, vLLM,
# or any other OpenAI-compatible upstream:
#
...
All routes mounted under `/v1`. No auth (the platform is reachable only on the compose network plus the host loopback for debugging). The shapes match the OpenAI public API on every field the rebuild's `OpenAICompatibleProvider` reads (`id`, `owned_by`, the SSE chunk format with `delta.content` / `delta.usage` / `finish_reason`). The wire path is `/v1/models` and the request body field is `model` because that's what the OpenAI SDK serialises — internally the rebuild and the platform both call each entry an "agent".

### `GET /v1/models`

Returns the configured `AgentDef` list, projected onto the OpenAI `models.list` wire shape. Each entry carries `id` (the agent alias) plus a non-standard `label` field the rebuild already understands.
...
Accepts the OpenAI request shape (`{model, messages, stream, temperature, ...}` — the body field is `model` on the wire; the rebuild's provider sets it from the chosen agent id). Extra fields are accepted (`extra="allow"`) since the rebuild's provider sends `stream_options` and `temperature` and may add more in M5.
...
- 404 `unknown model: <id>` — agent alias not in the configured catalogue. The wire-format error keeps the OpenAI vocabulary because the body field is `model`. The rebuild's `agents_cache.contains(...)` check normally prevents this from reaching the platform, but we return a clean 404 anyway.
- 400 `no user message in request` — message list contained no `user` role. Defensive only; the rebuild always sends one.
- 500 / 502 — anything else (Ollama down, OOM, etc.). The rebuild's provider wraps these into `ProviderError(status_code=502)` already, which surfaces to the SSE stream as a terminal `error` event per [m2-conversations.md § Streaming pipeline](m2-conversations.md#streaming-pipeline).
...
- **`test_agents.py`** — `build_agents(settings)` returns one entry per `AgentDef`; alias→tag mapping survives. `agents.get("does-not-exist") is None`.
- **`test_smoke_compose.py`** — `@pytest.mark.compose`, skipped by default. Brings up the stack via `docker compose up -d --wait`, curls `http://localhost:8080/api/agents` (through the rebuild app, not directly), asserts `{"items": [{"id": "dev", ...}]}` and that a streamed `POST /api/chats/{id}/messages` returns a non-empty assistant message. Run from `make verify-stack` only.
...
MSW handlers in [rebuild/frontend/src/lib/msw/handlers.ts](../../frontend/src/lib/msw/handlers.ts) keep returning the existing fixed agent catalogue; the agent platform is not used by Vitest, Playwright component tests, or Playwright E2E. The agent ids in the MSW handler list (`gpt-4o`, `gpt-4o-mini`, `claude-3-5-sonnet`) intentionally differ from the agent platform's (`dev`) — the MSW list represents what the production gateway eventually returns, not what dev ships locally.

## User journeys

_No product UI in this milestone._

This feature is dev-loop only. The agent-platform runs as a separate compose service; the runtime image and every production deploy are untouched. The one visible-surface artefact — the agent dropdown reading `Dev (Qwen 2.5, 0.5B)` in the dev environment — is a behavioural check on the M2-owned `AgentSelector` component, not a new UI surface. The M2 `AgentSelector-geometry.spec.ts` (owned by [m2 § User journeys](m2-conversations.md)) already asserts geometric invariants for that dropdown; no new row is needed here.
...
- [ ] `curl http://localhost:8081/v1/models` returns the configured catalogue (`dev`).
- [ ] `curl http://localhost:8081/v1/chat/completions -d '{"model":"dev","messages":[{"role":"user","content":"say hi"}],"stream":false}' -H 'Content-Type: application/json'` returns a non-empty assistant message. (The wire body uses the OpenAI field name `model`; in product terms `dev` is an agent id.)
- [ ] `curl -fsS -H "X-Forwarded-Email: dev@canva.com" http://localhost:8080/api/agents` (the rebuild's app, *through* the agent platform) returns `{"items": [{"id": "dev", ...}]}`.
- [ ] After `npm run dev`, `http://localhost:5173` renders the agent selector with one entry whose visible label reads `Dev (Qwen 2.5, 0.5B)` (not the bare alias `dev`) — proving the one-line `label` plumbing in `app/providers/openai.py` actually rounds the friendly label through to the dropdown.
- [ ] Picking the agent, typing "hello", and pressing send streams an assistant response into the conversation view; the response persists across page reload.
- [ ] Clicking cancel mid-stream stops the stream cleanly and persists the partial assistant message with the cancelled badge.
- [ ] `make verify-stack` passes (existing `/healthz` + `/readyz` + `/api/me` checks plus the new `/api/agents` non-empty assertion).
...
- **Per-agent system prompts**, tool wiring, or any non-trivial Pydantic AI usage. The plan deliberately ships agents with no system prompt and no tools — just a transport for completions. Future personas (`dev-pirate`, `dev-coder`, …) extend `agents.py` by reading an optional `AgentDef.system_prompt` field, without touching the OpenAI compat layer.
...
- **Automatic backing-tag swap on hardware constraints.** If `qwen2.5:0.5b` OOMs on a developer's tiny machine, they edit the `AGENTS` env block in compose. We do not auto-detect.
- **Production replacement.** The internal agent gateway in production stays the upstream for staging + prod (`AGENT_GATEWAY_BASE_URL` is injected by Helm). The agent platform never ships in the runtime image and never appears in any non-`infra/` path.
...
5. **Optional profile to skip the agent platform.** Not added — the agent platform is always brought up. Topology stays static; developers who prefer a real upstream override `AGENT_GATEWAY_BASE_URL` and (optionally) `docker compose stop` the unused services.

...
The M2 acceptance criteria re-baselined the backend LOC budget to 3,000 (see [m2-conversations.md § Acceptance criteria](m2-conversations.md#acceptance-criteria)) on the basis that the streaming-pipeline code is functionally correct, free of duplication, and validated by Phase 4a / 4c review plus the green integration suite. The two heaviest files (`app/services/chat_stream.py` ~840 LOC, `app/routers/chats.py` ~550 LOC) carry the SSE pipeline (six event types × four terminal branches × heartbeat tick × persist throttle) and the prepare/stream split that fixed the Starlette "response already started" race + leaked `SELECT FOR UPDATE` row lock. There is no active bug; chasing a refactor against a passing pipeline is not justified.

The optional refactor below is a low-risk safety valve, **not** an M6 acceptance gate. It lands only if the file is touched again for unrelated reasons (e.g. M5 chat-target write evolution surfaces a shared concern), at which point the reduction comes essentially for free as part of the larger change.

- **Optional: extract heartbeat + persist-throttle from `chat_stream.py` into a helper module.** The two concerns — emitting `: keep-alive\n\n` on quiet stretches via the `asyncio.wait_for` + `asyncio.shield` pattern, and the `monotonic() - last_persist > PERSIST_EVERY_S` checkpointing — are independent of the SSE event taxonomy and the terminal branches that own the bulk of the file. Pulling them into `app/services/streaming/heartbeat.py` and `app/services/streaming/persist.py` would let `stream_assistant_response` shrink to roughly the structural shape documented in `m2-conversations.md § Streaming pipeline` and would make both helpers reusable for any future SSE route that picks them up. Estimated reduction: ~150-200 LOC out of `chat_stream.py`. Tracking only; no acceptance criterion attached.
- **Optional: collapse the four `_close_with_*` terminal helpers if a shared shape emerges.** Each currently owns its own SSE frame + persist payload; if M6 sweeps reveal a common "close + persist + emit" pattern (e.g. all four want to record a structured `terminal_reason` for the zombie sweeper), a single `_close_terminal(reason, ...)` helper would replace four near-duplicates. Don't touch pre-emptively — the explicit branches are easier to reason about than premature abstraction.

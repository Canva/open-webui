...
class Automation(Base):
    ...
    model_id: Mapped[str] = mapped_column(String(128), nullable=False)  # width matches channel_message.bot_id (M3) so the channel-target write path never overflows; 128 chars covers every gateway model id observed in the wild
    ...
    # `back_populates` and avoid touching M0/M1/M3 models. The list-this-user's
...
`AutomationCreate.model_id` is also length-bounded with `Annotated[str, Field(min_length=1, max_length=128)]` to mirror the column width and the M3 `channel_message.bot_id` width — without this, a 200-character `model_id` would round-trip OK through `POST /api/automations` and only blow up at execute time when the channel-target write tried to populate `channel_message.bot_id` (`String(128)`). The 128-char ceiling matches every model id observed in the wild (longest known: `meta-llama/Meta-Llama-3.1-405B-Instruct-FP8` at ~52 chars) with comfortable headroom.

All three relationships on `Automation` (`user`, `target_chat`, `target_channel`) are one-way; no reverse-side `back_populates` is required on `User`, `Chat`, or `Channel`, so M0/M1/M3 models are untouched by this milestone. M4 never traverses `user.automations`, `chat.automations`, or `channel.automations` — the only "list automations for this user" query lives in the M4 router and is a direct `select(Automation).where(...)`.

## Alembic revision

- File: `rebuild/backend/alembic/versions/0005_m4_automations.py`
- `revision = "0005_m4_automations"`
- `down_revision = "0004_m3_channels"` (locked: M3's revision is pinned at `0004_m3_channels`; see M3 §Alembic revision).
- `depends_on = None`.
...
`alembic upgrade head`, `alembic downgrade -1`, **and a second `alembic upgrade head` immediately afterwards** must all succeed cleanly. Covered by the M0 `test_upgrade_head_is_idempotent` / `test_downgrade_base_is_idempotent` cases parametrised over `0005_m4_automations`. The targeted partial-recovery case in `test_partial_upgrade_recovers` pre-creates `automation` only (raw DDL, no indexes, no check), then runs `alembic upgrade head` and asserts both tables, all four named indexes, and both check constraints end up present.
...
3. Resolve context messages:
   - **Chat target.** Load `Chat.history` (the M1 JSON blob) and walk the `messages` tree from `currentId` back to root, in chronological order. The result is `[{role, content}, ...]` matching what the M1 SSE endpoint sends to the provider.
...
5. Call `provider.stream(messages=…, model=automation.model_id, params={})` — the same `OpenAICompatibleProvider` instance from M1, exposed via `app.state.provider`. Iterate the SSE chunks and accumulate the full assistant text into a single string. We are not relaying tokens to a browser here, so there is no need to spool through the M1 SSE adapter — we read the chunks and concatenate.
6. Persist the result based on target:
   - **Chat target.** Call the M1-owned helper `app.services.chat_writer.append_assistant_message(session, *, chat_id=automation.target_chat_id, parent_message_id=chat.history["currentId"], model=automation.model_id, content=full_response, status="complete")`. The helper atomically updates `chat.history.messages[<id>]`, sets `chat.history.currentId`, bumps `chat.updated_at`, and returns the new message id. The message structure (parentId, childrenIds, role, model, timestamp) matches what the M1 streaming endpoint produces, so the chat displays identically whether the response came from the user's browser or from a scheduled run. The `automation_run.chat_id` is set to the existing chat id.
   - **Channel target.** Call the M3-owned helper `app.services.channels.messages.create_bot_message(session, *, channel_id=automation.target_channel_id, bot_id=automation.model_id, content=ChannelMessageContent(text=full_response, mentions=[], attachments=[], embeds=[], edited=False, automation_id=automation.id, automation_owner_name=user.name), parent_id=None) -> ChannelMessage`. The helper does the DB insert (honouring the `(user_id IS NOT NULL) + (bot_id IS NOT NULL) + (webhook_id IS NOT NULL) = 1` CHECK), updates `channel.last_message_at`, and dispatches the realtime `message:create` emit through `app.realtime.events.emit_message_create`. **M4 does not call `sio.emit` directly and does not insert `channel_message` rows directly** — going through the M3 service is the only way to keep the persistence/realtime pairing consistent (M3 §Dependencies on other milestones). `automation_id` and `automation_owner_name` are part of the M3 `ChannelMessageContent` schema (M3 §3.4); the strict validator accepts them. The `automation_run.chat_id` is left `NULL`.
...
M4 extends the M0 `Settings` class with three new fields. The casing convention from M0 (UPPER_SNAKE_CASE attributes matching env var names) applies; access is `settings.AUTOMATION_*` everywhere in the scheduler/executor/router code.
...
| `AUTOMATION_TIMEOUT_SECONDS` | `int` | `120` | `asyncio.wait_for` timeout around `_execute`. Caps the worst-case impact of a misbehaving model on the scheduler. Independent of `SSE_STREAM_TIMEOUT_SECONDS` (M1) because the executor accumulates the response server-side and does not relay tokens to a browser. |
| `AUTOMATION_MIN_INTERVAL_SECONDS` | `int` | `300` | Floor on RRULE effective interval, validated on `POST /api/automations`. The 5-minute default exists to cap fan-out into the model gateway and to keep the scheduler tick budget tractable. The E2E suite lowers this to `60` so `FREQ=MINUTELY` rules can be tested without waiting; production must not lower it without re-running the M3/M4 load benchmark from `m5-hardening.md`. |
...
- `tests/e2e/automation-minutely.spec.ts`:
  ...
  7. Repeat steps 4–5 with a channel-target automation (created in a separate test) and assert the new `channel_message` arrives in a second `BrowserContext` via the M3 socket.io listener.
...
- **Hard dependency on M1** — `OpenAICompatibleProvider`, `Chat` table, `chat.history` JSON shape, the `app.services.chat_writer.append_assistant_message` helper (declared in M1 §Deliverables), and the `models` discovery used by the editor's model picker.
- **Hard dependency on M3** — channel target uses the `channel`, `channel_member`, `channel_message` tables, the `channel_message.bot_id` column (M3's design — no synthetic user is inserted; the row carries a non-null `bot_id` and null `user_id`/`webhook_id`), the optional `automation_id` / `automation_owner_name` fields on `channel_message.content` (M3 §3.4), and the M3-owned service helper `app.services.channels.messages.create_bot_message(...)` which performs the DB insert and the realtime `message:create` broadcast. M4 must call this helper rather than emitting socket events directly. **M3 must land before M4's Alembic revision applies**, because M4's `automation` model declares an inline FK from `target_channel_id` to `channel.id` — without `channel`, the migration fails at apply time. The milestone ordering in `rebuild.md` §0 (M3 → M4) already enforces this; do not attempt a "chat-only mode" workaround that defers the FK, because the inline declaration makes that incoherent. If a hard deadline ever forced M4 first, the right path would be to split M4 into M4a (chat-only, drops the `target_channel_id` column entirely) and M4b (adds the column + FK + UI in a follow-up revision once M3 lands), not to feature-flag a half-built schema.
...
- [ ] Alembic migration `0005_m4_automations` is reversible: `alembic upgrade head && alembic downgrade -1 && alembic upgrade head` round-trips on a fresh MySQL 8.0. Re-running `alembic upgrade head` immediately after `head` and re-running `alembic downgrade base` after `base` are both no-ops (covered by the M0 idempotency tests parametrised over the M4 revision).
- [ ] `test_partial_upgrade_recovers` includes an M4 case: pre-create `automation` only (raw DDL, no indexes, no check), then `alembic upgrade head` produces `automation_run`, all four named indexes, and both `ck_automation_*` check constraints without operator intervention.
...
- **Channel target authorship.** The post carries `bot_id=automation.model_id` (per M3) plus the `automation_owner_name` field in `content` so the UI can render "via @sam's automation 'Daily standup'". Confirm copy with design before launch.

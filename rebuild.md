...
| M4 | Channels (full Slack-shape). All channel tables, socket.io w/ Redis adapter, threads/reactions/pinned/members/typing/read-receipts/webhooks/file-uploads, `@model` auto-reply, port channel UI without dead imports | 3–4 weeks | [rebuild/docs/plans/m4-channels.md](rebuild/docs/plans/m4-channels.md) |
...
- **Theme FOUC.** A naive `localStorage`-only theme system flashes the wrong palette on first paint while JS hydrates, which is unacceptable on a tool people stare at all day. M1 mitigates with a cookie-driven server-rendered `<html data-theme="…">` and an inline boot script that runs *before* the SvelteKit hydration. The cookie + localStorage mirror is the contract; deviation re-introduces the flash. See [m1-theming.md § Persistence](rebuild/docs/plans/m1-theming.md#persistence).

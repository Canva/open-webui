...
3. From `rebuild/`, run the gates in order, stopping on the first hard failure: `make lint`, `make typecheck`, `make test-unit`, `make test-component`, then `make test-e2e-smoke` if any router or `(app)/` route changed. After the test gates pass, run the deployment-plumbing gates: `make verify-stack` (always — catches Dockerfile, compose, and boot-time regressions that lint/typecheck/tests can't see) and, when files under `backend/alembic/` or `backend/app/models/` changed, `make verify-migrate` (exercises the documented host-side `make migrate` flow against a fresh MySQL). Both targets are destructive — they tear down the `rebuild_*` compose stack and drop the mysql volume on entry and exit. Do not run them while the developer has work-in-progress data in the dev MySQL; if `docker compose ps` shows the rebuild stack already up before you run the gates, surface that to the parent and skip the destructive verifies rather than trample it.
